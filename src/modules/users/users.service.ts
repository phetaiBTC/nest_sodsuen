...
import { ITransactionManager } from 'src/common/transation/transition.interface';
...
export class UsersService {
  ...
  async create(createUserDto: CreateUserDto) {
    ...
    const isDistrict = await this.districtService.getOneById(
      createUserDto.districtId,
    );

    if (!isDistrict) throw new BadRequestException('District not found');
  ...
  async findAll() {
    const users = await this.userRepository.find({
      relations: ['role', 'district', 'district.province'],
    });
